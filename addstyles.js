const bodyClass = "bg-red-800 bg-gradient-to-br from-green-200 bg-opacity-25";
const bdy = document.querySelector('body');
bodyClass.split(" ").forEach(cls => bdy.classList.add(cls));

const calcBodyClass = "max-w-lg rounded overflow-hidden shadow-2xl bg-blue-900 bg-opacity-75 bg-gradient-to-tr from-gray-900 flex";
const calcBody = document.querySelector('.calculatorBody');
calcBodyClass.split(" ").forEach(cls => calcBody.classList.add(cls));


const outputClass = "flex flex-col w-3/4 h-12 justify-center items-end m-1 bg-gray-200 text-gray-900 font-semibold px-2 border border-white shadow-2xl hover:border-gray-500 rounded";
...
function addClasses(button){
    ...
    const baseClass = "flex w-12 h-12 justify-center items-align-center m-1 font-bold py-2 px-2 border-2 hover:border-transparent rounded text-white border-2 text-2xl"
